...
namespace SSD_Teams.Data
{
    public static class DbInitializer
    {
        ...
        private static async Task<int> SeedUsers(UserManager<ApplicationUser> userManager)
        {
            ...
            {
                UserName = "the.manager@teams.ca",
                Email = "the.manager@teams.ca",
                FirstName = "The",
                LastName = "Manager",
                EmailConfirmed = true
            };
            var result = await userManager.CreateAsync(adminUser, appSecrets.AdminPass);
            if (!result.Succeeded)
                return 1;  // should log an error message here
            ...
            {
                UserName = "the.player@teams.ca",
                Email = "the.player@teams.ca",
                FirstName = "The",
                LastName = "Player",
                EmailConfirmed = true
            };
            result = await userManager.CreateAsync(memberUser, appSecrets.MemberPass);
            if (!result.Succeeded)
                return 3;  // should log an error message here
